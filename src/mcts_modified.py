...
def expand_leaf(parent_node: MCTSNode, state, board):
    ...
    if len(parent_node.untried_actions) == 0:
        if DEBUG: print("Cant expand leaf there are no possıble plays proceed.")
...
def rollout(node:MCTSNode, state, board):
    ...
    if len(moves) == 0:
        ...
        if DEBUG: print("Cant rollout as there are no possıble plays proceed.")
...
def think(board, state):
    ...
    if DEBUG: print("{} is the heuristic bot".format(THIS_IDENTITIY))
